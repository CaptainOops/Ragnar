...
- **Sub-GHz RF instrument (RTL-SDR / HackRF)** — a real spectrum analyser in the browser: real-time IQ waterfall with adjustable display range, wheel/drag zoom, markers with Δ and peak search, band-plan labels and signal identification, dBm calibration, noise-print subtraction, limit lines with pass/fail alerts into Watchtower, unattended band surveys with reports, RSSI direction finding across mesh units, CSV/SigMF export and a listen bar (FM/NFM/AM/SSB/CW). See [RF Waterfall](docs/rf-waterfall.md), the [capability guide](docs/rf-waterfall-guide.md) and [SDR / Sub-GHz](docs/sdr-subghz.md).
...
> **[CREDITS.md](docs/CREDITS.md)** — Solarflere identified and curated the CVE corpus behind Ragnar's passive detection suite — **202 CVEs detected** from the wire, 241 named (CVE-2002-1623 → CVE-2026-81736). See the full writeup and numbers there, and the **[CVE Index](docs/CVE.md)** for every CVE with its detector and status.
